...
class ProblemeTransport:
    ...
    def charger_depuis_fichier(nom_fichier):
        ...
        if not chemin_fichier.exists():
            candidat_input = DATA_INPUT_DIR / nom_fichier
            if candidat_input.exists():
    ...
    def methode_marche_pied_potentiels(
        ...
        if initialisation_deja_faite:
            if not self.base:
                raise ValueError(
                    "initialisation_deja_faite=True exige un problème déjà initialisé en transport/base."
                )
            init = ""
            nom_init = f"{methode_initiale} (déjà initialisée)"
        else:
